"""Piecewise Landau approximation used by asymptotic combinations.

Adapted coefficient algorithms: Copyright (c) 2005, LCG ROOT MathLib Team.
ROOT MathCore is LGPL-2.1-or-later; see THIRD_PARTY.md and bundled licenses.
The Python translation uses NumPy arrays and this package's S1 parameters.
"""
...
class Landau:
  """Landau distribution approximation used for large combinations."""

  @classmethod
  def pdf(cls, x: float | np.float64 | np.ndarray, loc: float | np.float64 | np.ndarray=0.0, scale: float | np.float64 | np.ndarray=1.0) -> float | np.float64 | np.ndarray:
    return (
      cls.__base_pdf(
        (x - loc) * np.pi / (2 * scale)
  ...
  @classmethod
  def cdf(cls, x: float | np.float64 | np.ndarray, loc: float | np.float64 | np.ndarray=0.0, scale: float | np.float64 | np.ndarray=1.0) -> float | np.float64 | np.ndarray:
    return cls.__base_cdf(
      (x - loc) * np.pi / (2 * scale) + np.log(np.pi / (2 * scale))
    )

  @classmethod
  def sf(cls, x: float | np.float64 | np.ndarray, loc: float | np.float64 | np.ndarray=0.0, scale: float | np.float64 | np.ndarray=1.0) -> float | np.float64 | np.ndarray:
    return 1 - cls.cdf(x, loc, scale)

  @classmethod
  def ppf(cls, x: float | np.float64 | np.ndarray, loc: float | np.float64 | np.ndarray=0.0, scale: float | np.float64 | np.ndarray=1.0) -> float | np.float64 | np.ndarray:
    return (
  ...
  @staticmethod
  def __base_pdf(x: float | np.float64 | np.ndarray) -> float | np.float64 | np.ndarray:
  ...
  @staticmethod
  def __base_cdf(x: float | np.float64 | np.ndarray) -> float | np.float64 | np.ndarray:
  ...
  @staticmethod
  def __base_ppf(x: float | np.float64 | np.ndarray) -> float | np.float64 | np.ndarray:
    f = np.array([
      0,
      0,
      0,
